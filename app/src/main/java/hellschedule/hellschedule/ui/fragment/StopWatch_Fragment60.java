...
public class StopWatch_Fragment60 extends Fragment {

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup v = (ViewGroup) inflater.inflate(R.layout.fragment_timer60, container, false);

        //region ImageView 형태의 splash GIF 변수를 만들고 Glide로 gif파일 불러오기
        ImageView splashGif = (ImageView)v.findViewById(R.id.timer60);
        GlideDrawableImageViewTarget gifImage = new GlideDrawableImageViewTarget(splashGif);
        Glide.with(getContext()).load(R.raw.timer60).into(splashGif);
    ...
    public static StopWatch_Fragment60 newInstance() {
        return new StopWatch_Fragment60();
    }
}
