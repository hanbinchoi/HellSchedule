...
public class BMI1_Fragment extends Fragment {

    public static View view_BMI_Exercise_Fragment;
    public static RecyclerView recyclerView_bmi;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        recyclerView_bmi = (RecyclerView) view_BMI_Exercise_Fragment.findViewById(R.id.recycler_view_bmi);
        layoutManager = new LinearLayoutManager(getActivity().getApplicationContext());
        recyclerView_bmi.setLayoutManager(layoutManager);
        recyclerView_bmi.setHasFixedSize(true); // recyclerView 사이즈 고정

        //init DB
        database = new Database(getActivity().getApplicationContext());

        adapter = new RecyclerViewAdapter_BMI(getActivity().getApplicationContext(), database.get_BMI1_List());
        recyclerView_bmi.setAdapter(adapter);

        return view_BMI_Exercise_Fragment;
    }

    //Activity 하위의 모든 Fragment들이 newInstance() 메소드를 가지고 있어야 화면 전환이 가능하다.
    public static BMI1_Fragment newInstance() {
        return new BMI1_Fragment();
    }
}
